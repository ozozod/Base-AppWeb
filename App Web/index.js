...
/*
* NOTA: Para que la nueva funcionalidad de "Dispositivos" funcione,
* es necesario crear la siguiente tabla en tu base de datos PostgreSQL:
...
// =================================================================
// === AUTENTICACIÓN Y HELPERS (PARA RUTAS MÓVILES) ================
// =================================================================

// Helper para firmar tokens JWT
...
// Middleware para verificar el token y adjuntar datos a req.auth
const requireAuth = (req, res, next) => {
  const { authorization } = req.headers;
  if (!authorization || !authorization.startsWith('Bearer ')) {
    // Permite continuar sin autenticación para rutas que no la requieran estrictamente
    return next();
  }
  ...
  try {
    ...
    req.auth = payload; // Adjunta { userId, role, eventId }
    next();
  } catch (error) {
    // Si el token es inválido, no adjuntamos nada. Las rutas decidirán qué hacer.
...
// =================================================================
// === RUTAS PARA APP MÓVIL (MODULARIZADO) =========================
// =================================================================
...
app.delete('/api/events/:eventId', async (req, res) => {
    ...
    try {
        ...
        // Asumiendo que las FK tienen ON DELETE CASCADE, solo necesitamos eliminar el evento.
        // Si no, necesitamos eliminar en orden: transacciones, productos, staff, customers, y finalmente evento.
        await client.query('DELETE FROM device_status WHERE event_id = $1', [eventId]);
...
app.listen(PORT, HOST, () => {
  ...
  console.log(`Para conectar desde otros dispositivos, usa la IP de tu red local.`);
});
